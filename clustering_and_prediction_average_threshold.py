...
for _, y in [pt for cluster in formatted_clusters for pt in cluster]:
    y_counts[y] += 1
...
for key, image in resized_boxes.items():
    cv2.imwrite(f'output_binarized/box_{key}.png', image)
...
directory_path = "output_binarized/" 
predicted_string, _ = predict_letters_from_directory(directory_path, loaded_model, class_names)
